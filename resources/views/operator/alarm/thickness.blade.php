...
<div class="filter-toolbar">
    <form method="GET" action="{{ route('alarm.thickness') }}" class="filter-form" id="filterForm" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: flex-end; width: 100%;">
        ...
        <div class="filter-actions-compact" style="margin-left: auto;">
            ...
            <a href="{{ route('alarm.thickness') }}" class="btn-reset-compact">
        ...
        <tbody>
            @foreach($failedTests as $test)
                <tr class="table-row-{{ $loop->odd ? 'odd' : 'even' }}">
                    <td>{{ $test->thickness_date_log->format('M d, Y') }} {{ $test->thickness_time_log }}</td>
                    <td><strong>{{ $test->operator_name }}</strong></td>
                    <td>{{ $test->production_line_name }}</td>
                    <td>{{ $test->shift_name }}</td>
                    <td>{{ $test->plate_code }}</td>
                    <td>{{ $test->plateSpecification?->thick_lsl ?? '—' }}</td>
                    <td>{{ $test->plateSpecification?->thick_target ?? '—' }}</td>
                    <td>{{ $test->plateSpecification?->thick_usl ?? '—' }}</td>
                    <td>{{ $test->op_c1 ?? '—' }}</td>
                    <td>{{ $test->op_c2 ?? '—' }}</td>
                    <td>{{ $test->op_c3 ?? '—' }}</td>
                    <td>{{ $test->op_c4 ?? '—' }}</td>
...
<div class="empty-state">
    ...
    <p>No failed thickness measurements match your selected filters.</p>
    @if(request()->anyFilled(['from_date', 'to_date', 'operator', 'line', 'shift', 'plate_code']))
        <a href="{{ route('alarm.thickness') }}" class="empty-state-link">
...
class FilterManager {
    ...
    initEventListeners() {
        ...
        document.querySelector('.btn-clear-all')?.addEventListener('click', (e) => {
            e.preventDefault();
            window.location.href = '{{ route("alarm.thickness") }}';
        });
    }
