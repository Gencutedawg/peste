...
class ThicknessAlarmController extends Controller
{
    public function thickness(Request $request)
    {
        $query = PlateThicknessLog::where('plate_quality_status_id', 2)->with('plateSpecification');

        if ($request->filled('from_date')) {
            $query->whereDate('thickness_date_log', '>=', $request->from_date);
        }

        if ($request->filled('to_date')) {
            $query->whereDate('thickness_date_log', '<=', $request->to_date);
        }
        ...
        $failedTests = $query->orderBy('thickness_date_log', 'desc')
            ->orderBy('thickness_time_log', 'desc')
            ->paginate($perPage)
            ->appends($request->query());

        $totalFailed = PlateThicknessLog::where('plate_quality_status_id', 2)->count();
        ...
        return view('operator.alarm.thickness', compact(
            'failedTests',
            'totalFailed',
            'operators',
            'lines',
